...
import com.newland.alipayService.common.captcha.CaptchaSecurity;
...
import com.newland.alipayService.txn.TxnErrorCode;
import com.newland.alipayserver.web.annotation.ExecuteResult;
import com.newland.alipayserver.web.view.AuthCardView;
...
public class BankCardManagerAction extends BaseAction {
	...
	public String addAcct() throws AppBizException, InterruptedException {
		...
		if (bindCard != null) {
			this.addActionMessage("亲，这张卡已和您当前的支付宝服务窗绑定过了哦");
			return "toAddCard";
		}
		// 2 验证账号 和类型
		if (AccountUtil.isCreditCard(authCardView.getAcctNo())) {
			 ...
			 * 我的意思 没让做信用卡绑定啊
			 */
			this.addActionMessage("暂不支持信用卡绑定");
	...
	public String auth() throws Exception {
		...
		if (blackList != null) {
			if (blackList.getApplyErr() >= BlackListService.BLACK_LIST_AUTH_CARD_MAI) {
				this.addActionMessage("您的支付宝服务窗绑定服务今天已被锁定，请您明天再尝试");
				return "toAddCard";
			}
		}
		...
		try {
			...
			logger.error("绑定支付宝服务窗,验证密码异常" + e.getMessage(), e);
			...
			if (e instanceof CoreNativeException) {
				...
				logger.debug("绑定支付宝服务窗,验证密码，错误码：" + errcode);
				// 信用卡
	...
	public String openNotify() throws Exception {
		...
		if (blackList != null) {
			if (blackList.getApplyErr() >= BlackListService.BLACK_LIST_NOTIFY_MAI) {
				this.addActionMessage("您的支付宝服务窗动户通知开通服务今天已被锁定，请您明天再尝试");
				return "show";
			}
		}
