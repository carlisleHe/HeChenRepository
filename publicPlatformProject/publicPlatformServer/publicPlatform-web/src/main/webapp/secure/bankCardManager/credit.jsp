<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>
<%@ include file="/common/meta.jsp"%>
<html>
<head>
<title><s:text name="APP_NAME"/></title>
<%@ include file="/common/header.jsp"%>
<script>
var opType;
function tanChu(obj,msg,type) {
	opType = type;
	$('.tczz').show().css('height', $(document).height());
	$("#tccnr2").html(msg);
	$(obj).show().css({
		'top' : ($(document).height() / 2) - ($('.tccbox').height() / 2),
		'left' : ($(document).width() / 2) - ($('.tccbox').width() / 2)
	});
}
function tan(obj) {
	$('.tczz').hide();
	$(obj).hide();
	if(opType==1){
		$("#closeform").unbind('submit').submit();
	}
	if(opType==2){
		$("#defaultform").unbind('submit').submit();
	}
	if(opType==3){
		$("#notifyform").unbind('submit').submit();
	}
}
function tanCN(obj) {
	$('.tczz').hide();
	$(obj).hide();
}
</script>
</head>

<body>
	<div class="wrap">
			<div style="padding-top: 20px;"></div>
			<ul class="lisbox">
				<li>
					<span class="bdico">
						<img src="resources/images/usico.png" alt="" />
					</span>
					<span class="bdzd fl">账户名<s:property value="selAuthCardView.acctName"/></span> 
					<s:if test="selAuthCardView.defaultCard">
						<span class="zswz hswz">默认信用卡</span>
					</s:if>
				</li>
				<li>
					<span class="bdico">
						<img src="resources/images/bqyh.png" alt="" />
					</span>
					<span class="bdzd fl">本期应还款</span>
					<div class="fr">
						<span class="zswz"><s:property value="%{getText('format.money',{selAuthCardView.billAmountCNY})}"/></span><span class="zswz hswz">人民币</span><br>
						<span class="zswz"><s:property value="%{getText('format.money',{selAuthCardView.billAmountUSD})}"/></span><span class="zswz hswz">美元</span>
					</div>
				</li>
				<li class="last">
					<span class="bdico">
						<img src="resources/images/dqhk.png" alt="" />
					</span>
					<span class="bdzd fl">到期还款日</span>
					<s:if test="selAuthCardView.endRefundDate != null">
						<span class="zswz hswz">每个月<s:property value="selAuthCardView.endRefundDate"/>号</span>
					</s:if>
					<s:else>
						<span class="zswz hswz">没有查询到</span>
					</s:else>
				</li>
			</ul>
			<ul class="lisbox ljlist">
						<s:form id="closeform" action="authorization!close.do" namespace="" method="post">
							<li>
								<a href="#"  onClick="tanChu('.tc2','解除绑定后可以重新绑定，您确定要解绑吗？',1)">
									<span class="bdico">
										<img src="resources/images/jcbd.png"   alt="" />
									</span>
									<span class="bdzd fl">解除和<s:text name="APP_NAME"/>绑定</span>
									<span class="zswz hswz">
										<img src="resources/images/ljico.png"  alt="" />
									</span>
								</a>
							</li>
						</s:form>
						<s:if test="selAuthCardView.defaultCard">
							<li>
									<span class="bdico">
										<img src="resources/images/mrico.png" style="opacity:0.20;"  alt="" />
									</span>
									<span class="bdzd fl" style="color:#CCCCCC">默认卡</span>
									<span class="zswz hswz">
										<img src="resources/images/ljico.png" style="opacity:0.20;"  alt="" />
									</span>
							</li>
						</s:if>
						<s:else>
						<s:form id="defaultform" action="authorization!defaultCard.do" namespace="" method="post">
							<li>
								<a href="#" onClick="tanChu('.tc2','您确定将此卡设置成默认信用卡了吗？',2)">
									<span class="bdico">
										<img src="resources/images/mrico.png" alt="" />
									</span>
									<span class="bdzd fl">设置为默认卡</span>
									<span class="zswz hswz">
										<img src="resources/images/ljico.png" alt="" />
									</span>
								</a>
							</li>
							</s:form>
						</s:else>
						<s:form id="notifyform" action="authorization!openNotify.do" namespace="" method="post">
							<li class="last">
								<a href="#"  onClick="tanChu('.tc2','您确定<s:if test="selAuthCardView.NotifyType==00">关闭</s:if><s:else>开通</s:else><s:text name="APP_NAME"/>动户通知了吗？',3)">
									<span class="bdico">
										<img src="resources/images/tzico.png" alt="" />
									</span>
									<span class="bdzd fl">
										<s:if test="selAuthCardView.NotifyType==00">关闭</s:if><s:else>开通</s:else><s:text name="APP_NAME"/>动户通知 	
									</span>
									<span class="zswz">
										<img src="resources/images/ljico.png" alt="" />
									</span>
								</a>
							</li>
						</s:form>
				</ul>
			   <s:form id="showform" action="authorization!list.do" namespace="" method="post">
					<div class="btnbox">
						<s:submit value="返回" cssClass="nxtbt"></s:submit>
					</div>
					<div class="foot"></div>
				</s:form>
	</div>
		<div class="tccbox tc2">
		  <div class="tcctit">信息提示</div>
		  <div class="tccnr" id="tccnr2"></div>
		  <div class="tccbtn">
		    <input type="button" onClick="tanCN('.tc2')" class="btn cal" value="有点犹豫">
		    <input type="button" onClick="tan('.tc2')" class="btn con" value="非常确定">
		  </div>
		</div>
    <%@include file="/common/foot.jsp"%>
</body>
</html>