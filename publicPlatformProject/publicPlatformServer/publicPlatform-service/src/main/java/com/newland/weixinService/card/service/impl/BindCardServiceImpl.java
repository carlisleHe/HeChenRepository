package com.newland.weixinService.card.service.impl;
...
import com.newland.weixinService.card.dao.BindCardDao;
import com.newland.weixinService.card.model.BindCard;
import com.newland.weixinService.card.service.BindCardService;
import com.newland.weixinService.customer.model.Customer;
import com.newland.weixinService.customer.service.CustomerService;
...
public class BindCardServiceImpl implements BindCardService{
	...
	@Override
	@Transactional("weixinTransactionManager")
	...
	private AuthNotify fillAuthNotity(BindCard bindCard){
		...
		authNotify.setAppId(WeixinApplication.getOpenbankId());
		...
		try{
			if(queryAuthCard == null){
				...
				if(Const.STATUS_INVALID.equals(queryAuthCard.getValid())){
					...
					//3、解绑通知信息
					//查询是否开通动户通知
					List<AuthNotify> authNotifys = authNotifyService.findAuthNotifyByAcctNo(WeixinApplication.getOpenbankId(), queryAuthCard.getOpenId(), queryAuthCard.getAcctNo());
